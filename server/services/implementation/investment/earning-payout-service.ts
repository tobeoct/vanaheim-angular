...
//-> Get EarningPayouts Made up until the end of last cycle and deduct from loan amount
...
export class EarningPayoutService extends BaseService<EarningPayout> implements IEarningPayoutService {
  constructor(private _db: any, private _customerRepository: CustomerRepository, private _templateService: TemplateService, private _loanRequestLogRepository: ILoanRequestLogRepository, private _loanRequestRepository: ILoanRequestRepository, private _approvedEarningRepository: IApprovedEarningRepository, private _earningPayoutRepository: IEarningPayoutRepository, private _emailService: EmailService, private _appConfig: AppConfig, private _utils: UtilService) {
    super(_earningPayoutRepository)
  }
  getByApprovedEarningID = (approvedEarningId: number) => new Promise<any[]>(async (resolve, reject) => {

    try {
      let repayments = await this._earningPayoutRepository.getByApprovedEarningID(approvedEarningId) as EarningPayout[];
      resolve(repayments);
    } catch (err: any) {
  ...
  processEarningPayout = ({ amount, approvedEarningId, loanRequestId, nextPayment, nextPaymentDate, dateOffset = 0 }: any) => new Promise<Response<string>>(async (resolve, reject) => {
    ...
    try {
      ...
      let repaymentsSoFar = await this._earningPayoutRepository.getByApprovedEarningID(approvedEarningId);
      let currentCycleEarningPayouts = this.reducePayments(repaymentsSoFar, moment(nextPaymentDate ?? approvedEarning.nextPaymentDate), moment(nextPaymentDate ?? approvedEarning.nextPaymentDate).subtract(1, this.getDenom(loanRequest.denominator)));
      console.log("Current Cycle EarningPayouts", currentCycleEarningPayouts)
      let totalEarningPayoutSoFar = this.reducePayments(repaymentsSoFar, moment(approvedEarning.maturityDate));//this.reducePayments(await this._earningPayoutRepository.getByApprovedEarningID(approvedEarningId),period.from.add(dateOffset,this.getDenom(loanRequest.denominator)));

      console.log("EarningPayouts so far", totalEarningPayoutSoFar)
      let expectedFullPayment = loanRequest.monthlyPayment;
      ...
      if (totalEarningPayoutSoFar != expectedFullPayment) {
        let repayment = new EarningPayout();
        repayment.code = this._utils.autogenerate({ prefix: "RPY" });
        repayment.amount = (currentCycleEarningPayouts + amount) > approvedEarning.nextPayment ? approvedEarning.nextPayment - currentCycleEarningPayouts : amount;
        repayment.datePaid = dateOffset > 0 ? moment().add(dateOffset, this.getDenom(loanRequest.denominator)).toDate() : new Date();
        repayment.approvedEarningID = approvedEarningId;
        repayment.earningRequestId = loanRequestId;
        repayment.status = BaseStatus.Active;
        repayment.accountNumber = "";
        repayment.payoutType = amount < approvedEarning.nextPayment ? PaymentType.Partial : PaymentType.FullPayment;
        approvedEarning.earningStatus = ApprovedEarningStatus.OnTrack;
        await this._approvedEarningRepository.update(approvedEarning);
        let repaymentInDb = await this._earningPayoutRepository.create(repayment);
      }
      ...
      if (totalEarningPayout > approvedEarning.nextPayment) {
        ...
        let repaymentCycle = await this.getEarningPayoutHealth(approvedEarningId);
        let nextPayment = repaymentCycle.data.filter(c => c.rawDate > approvedEarning.nextPaymentDate).sort(this.sortInDesc("rawDate"))[0];
        ...
        console.log("Disbursed Loan ", d.nextPaymentDate);
        ...
        if (rollOver > 0) {
          let result = await this.processEarningPayout({
            ...
            nextPaymentDate: approvedEarning.nextPaymentDate, amount: rollOver, approvedEarningId, loanRequestId, dateOffset: dateOffset += 1
          });
          resolve(result);
          return;
        }
      ...
      resolve({ status: true, data: "EarningPayout was successful" });
      return;
    } catch (err: any) {
      resolve({ status: false, data: "EarningPayout was not successful" });
      return;
    }
  });
  processEarningPayoutPlan = ({ email, tenure, denominator, loanType, purpose, rate, loanAmount, monthlyEarningPayout }: any, userData: any) => new Promise<any>(async (resolve, reject) => {
    ...
    try {
      ...
      let t = this.getEarningPayoutTemplate({ tenure: tenure + ' ' + denominator, loanType, purpose, rate, loanAmount, monthlyEarningPayout });
      let { path }: any = await this._templateService.generatePDF("EarningPayout Plan", [], "repayments/" + fileName, t);
  ...
  getPastCycles(repayments: EarningPayout[], dateFunded: Date, lastCyclePeriod: number, denominator: string) {
    ...
    for (let i = 0; i < (lastCyclePeriod - 1); i++) {
      ...
      pastCycles.push({ paymentDate: dueDate, rawDate: to.toDate(), loanBalance, principalEarningPayout: monthlyPrincipal, interestEarningPayout: monthlyInterest, loanEarningPayout: monthlyPayment })
      loanBalance = loanBalance - monthlyPrincipal;
    }

    return pastCycles;
  }
  generateHealth = (repaymentCycles: any[], currentEarningPayouts: EarningPayout[]): EarningPayoutHealth[] => {
    return [];
    ...
    //   let status = moment().toDate() > r.rawDate ? amountRepaid < r.loanEarningPayout ? "Defaulted" : "Fully Repaid" : (amountRepaid < r.loanEarningPayout) && this.isWithinCurrentCycle(moment(r.rawDate).subtract(1, "month").toDate(), r.rawDate) ? amountRepaid == 0 ? "Awaiting First Payment" : "Partial" : amountRepaid >= r.loanEarningPayout ? "Fully Paid" : "Not Yet Due"
    //   return { dueDate: r.paymentDate, rawDate: r.rawDate, expectedPayment: r.loanEarningPayout, amountRepaid, status, lastPayment: lastEarningPayout?.amount ?? 0, lastPaymentDate: lastEarningPayout?.dateRepaid };
    // })
  ...
  getLastEarningPayoutPerCycle(repayments: EarningPayout[], from: Date, to: Date) {
  ...
  getEarningPayoutHealth = (approvedEarningId: number) => new Promise<Response<EarningPayoutHealth[]>>(async (resolve, reject) => {
    try {
      ...
      if (!approvedEarning) { console.log("getEarningPayoutHealth => Cant find Disbursed Loan"); resolve({ status: true, data: [] }) }

      let currentEarningPayouts = await this._earningPayoutRepository.getByApprovedEarningID(approvedEarningId);
      // let { tenure, loanType, rate, denominator, amount, dateApproved, monthlyPayment } = approvedEarning.EarningRequest;
      // let lastPeriod = this.lastPaymentCyclePeriod(dateApproved, denominator);
      // let t = tenure - lastPeriod;
      // const loanAmount = amount - this.getPaymentsTillLastCycleEnd(currentEarningPayouts, dateApproved, denominator);
      // let pastCycles = this.getPastCycles(currentEarningPayouts, dateApproved, lastPeriod, denominator);
      // let remainingCycles = this.generateEarningPayoutCycle({ dateFunded: moment(dateApproved).add(lastPeriod, this.getDenom(denominator)), tenure: t, denominator, rate: rate / 100, loanAmount, monthlyEarningPayout: this.calculateMonthlyEarningPayout(loanAmount, t, loanType) });
  ...
  calculateMonthlyEarningPayout = (loanAmount: any, tenure: any, loanType: any) => {
  ...
  getPaymentsTillLastCycleEnd(repayments: EarningPayout[], dateFunded: Date, denominator: string): number {
  ...
  private reducePayments(repayments: EarningPayout[], to: moment.Moment, from?: moment.Moment): number {
    return 0;
    
    // repayments.reduce((total, repayment) => {
  ...
  generateEarningPayoutCycle({ tenure, denominator, rate, loanAmount, monthlyEarningPayout, dateFunded }: any): EarningPayoutCycle[] {

    // let p = Number(loanAmount.replace(/[^0-9.-]+/g, ""));
    let p = loanAmount;
    let loanEarningPayout = monthlyEarningPayout.toString();
    let i = rate;
    let period = "month";
    ...
    // loanEarningPayout = Number(loanEarningPayout.replace(/[^0-9.-]+/g, ""));
    let loanBalance = p;
    ...
    for (var c = 0; c < tenure; c++) {
      ...
      monthlyPrincipal = loanEarningPayout - monthlyInterest;
      const options: any = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
      ...
      repayments.push({ paymentDate: dueDate, rawDate: today.toDate(), loanBalance, principalEarningPayout: monthlyPrincipal, interestEarningPayout: monthlyInterest, loanEarningPayout })
      loanBalance = loanBalance - monthlyPrincipal;
    }

    return repayments;
  }
  private getEarningPayoutTemplate({ tenure, loanType, purpose, rate, loanAmount, monthlyEarningPayout }: any): string {
    let template = "";
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" style="width:100% !important; font-size:12px !important; border:1px solid #e0e0e0;">
                    <thead style="width:100%;">
                      <tr style="text-align:center; width:100%; color:#E6AF2A; background:#333333; padding-top:10px; padding-bottom:10px;">
                        ...
                        <th style="padding-top:10px; padding-bottom:10px;">Loan EarningPayout <b>(NGN)</b></th>
                      </tr>
                    </thead>
                    <tbody>
                   `;
