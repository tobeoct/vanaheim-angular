...
export class EarningDragAndDropComponent implements OnInit {
  ...
  handleUpdate = (event: any, id: any, classList: any) => {
    return () => {
      ...
      this._requestService.updateStatus(id, this._requestService.getStatus(classList), (this.getStatus(this.classListSubject.value) == "Active") ? undefined : this.failureReason.value, this.mailMessage.value, this.startDate.value,this.serialNumber.value).then(c => {
