...
import { EarningsPersonalInfo } from './personal-info';
...
export class PersonalInfoComponent implements OnInit {
  ...
  constructor(private _router: Router, private _utility: Utility, private _fb: FormBuilder, private _store: Store,private _earningsStore:EarningsStore, private _customerService: CustomerService,
  ...
  ngOnInit(): void {
    ...
    let max = year - 18;
    const personalInfo = this._earningsStore.personalInfo as EarningsPersonalInfo ;
    this.form = this._fb.group({
      ...
      taxId: [personalInfo.taxId ? personalInfo.taxId : ""],
    ...
    this._earningsStore.titleSubject.next("Personal Information");
  ...
  patchValue(customer: any) {
    ...
    if (customer) {
      ...
      if (!this.taxId.value && customer.taxId) {
        this.taxId.patchValue(customer.taxId);
      }
  ...
  onSubmit = (form: FormGroup) => {
    ...
    const personalInfo: EarningsPersonalInfo = { title: this.title.value, surname: this.surname.value,taxId:this.taxId.value, gender: this.gender.value, maritalStatus: this.maritalStatus.value, firstName: this.firstName.value, otherNames: this.otherNames.value, email: this.email.value, phoneNumber: this.phone.value, dob: { day: this.day.value, month: this.month.value, year: this.year.value }, address: { street: this.street.value, city: this.city.value, state: this.state.value } };
    this._earningsStore.setPersonalInfo(personalInfo);
    if (!this.side) {
